  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">Assessment Management</h1>
          <p className="text-[10px] text-slate-500 mt-1 font-bold uppercase tracking-widest italic">Monitor, publish, and share your quizzes.</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="formal-card p-20 text-center border-dashed border-slate-700">
          <div className="text-4xl mb-6 grayscale">📭</div>
          <h3 className="text-sm font-bold text-white uppercase tracking-widest">No Quizzes Active</h3>
          <p className="text-slate-500 max-w-sm mx-auto mt-2 text-[10px] font-bold uppercase tracking-tighter">Your quiz catalog is empty. Create an assessment to begin.</p>
        </div>
      ) : (
                ...
                >
                  <div className="flex items-start justify-between gap-4 mb-5">
                    <div className="flex-1">
                      <h3 className="font-bold text-md text-white tracking-tight leading-tight">{quiz.title}</h3>
                      <div className="flex flex-wrap items-center gap-2 mt-2">
                        <span className="text-[8px] font-bold px-2 py-0.5 rounded-sm bg-slate-800 text-slate-400 border border-slate-700 uppercase">
                          {quiz.department}
                        </span>
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-4 border-t border-slate-800">
                    <button onClick={(e) => { e.stopPropagation(); handleQRCode(quiz._id); }}
                      className="flex-1 flex items-center justify-center gap-2 text-[9px] font-bold uppercase tracking-widest px-3 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 transition-all text-slate-300">
                      View QR
                    </button>
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-10">
            {qrCode ? (
              <div className="formal-card p-8 text-center animate-fade-in relative overflow-hidden">
                ...
                <h3 className="text-[9px] font-bold uppercase tracking-[0.3em] mb-8 text-slate-500">Distribution Console</h3>
                
                <div className="bg-white p-6 rounded-xl mx-auto w-fit mb-8 shadow-inner ring-4 ring-slate-900">
                  <img src={qrCode.qrCode} alt="Quiz QR Code" className="w-40 h-40 sm:w-56 sm:h-56" />
                </div>

                <div className="bg-slate-900 border border-slate-800 rounded-lg p-4 mb-8 text-left">
                   <p className="text-[8px] font-bold text-slate-500 uppercase tracking-widest mb-1">Entry Point URL</p>
              ...
              <div className="formal-card p-12 text-center border-dashed border-slate-700 min-h-[400px] flex flex-col items-center justify-center">
                <div className="w-12 h-12 rounded-xl bg-slate-900 flex items-center justify-center text-2xl mb-4 grayscale opacity-30">📱</div>
                <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.2em] max-w-[180px] leading-relaxed">Select an entry from the catalog to initialize sharing</p>
              </div>
            )}
          </div>
        </div>
